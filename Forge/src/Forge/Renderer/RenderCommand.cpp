...
void RenderCommand::Draw(const std::shared_ptr<VertexArrayBuffer>& buffer, DrawPrimitives primitives) {
    buffer->Bind();
...
void RenderCommand::DrawIndexed(const std::shared_ptr<VertexArrayBuffer>& buffer, uint32_t indexCount, DrawPrimitives primitives) {
    buffer->Bind();
