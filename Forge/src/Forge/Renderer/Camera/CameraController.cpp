...
void CameraController::Update(std::shared_ptr<Camera>& camera) {
    if (Keyboard::IsKeyPressed(Key::LeftShift)) {
        if (Keyboard::IsKeyPressed(Key::LeftMouse)) {
            ...
            camera->m_Target += (float)mouse_delta.first * (m_Right * camera->m_MovementSpeed) * (camera->m_Radius);
            camera->m_Position += (float)mouse_delta.first * (m_Right * camera->m_MovementSpeed) * (camera->m_Radius);

            camera->m_Target += (float)mouse_delta.second * (m_Up * camera->m_MovementSpeed) * (camera->m_Radius);
            camera->m_Position += (float)mouse_delta.second * (m_Up * camera->m_MovementSpeed) * (camera->m_Radius);
        } else {
        ...
        if (Keyboard::IsKeyPressed(Key::LeftMouse)) {
            ...
            camera->m_Position =
                camera->m_Target +
                glm::vec3(camera->m_Radius * cos(glm::radians(camera->m_RotationYaw)) * cos(glm::radians(camera->m_RotationPitch)),
                          camera->m_Radius * sin(glm::radians(camera->m_RotationPitch)),
                          camera->m_Radius * sin(glm::radians(camera->m_RotationYaw)) * cos(glm::radians(camera->m_RotationPitch)));

            glm::vec3 front;
            front.x = cos(glm::radians(camera->m_RotationYaw)) * cos(glm::radians(camera->m_RotationPitch));
            front.y = sin(glm::radians(camera->m_RotationPitch));
            front.z = sin(glm::radians(camera->m_RotationYaw)) * cos(glm::radians(camera->m_RotationPitch));
