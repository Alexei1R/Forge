...
enum class CullFace : uint32_t { NONE = 0, FRONT, BACK, FRONT_AND_BACK };

enum class BlendingMode : uint32_t { NONE = 0, ADD, SUBTRACT, REVERSE_SUBTRACT };
...
struct Material {
    ...
    std::unordered_map<std::string, MaterialParamValue> AdditionalParameters;
    ...
    std::optional<T> GetParameter(const std::string& name) const {
        auto it = AdditionalParameters.find(name);
