...
void Column::AddChild(std::shared_ptr<Widget> child) {
    try {
        ...
        LOG_ERROR("Column::AddChild - shared_from_this() failed. Ensure Column is "
                  "managed by "
                  "shared_ptr.");
        throw;
    }
...
const DrawListData Column::GetDrawList() {
    ...
    for (auto& child : m_Children) {
        ...
        advanceY += child->GetSize().y + static_cast<int>(padding.y) + static_cast<int>(padding.w); // padding.w = bottom
        ...
        int childTotalWidth =
            child->GetSize().x + static_cast<int>(padding.x) + static_cast<int>(padding.z); // padding.x = left, padding.z = right
